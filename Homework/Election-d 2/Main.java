public class Main {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < candidates.length; i++) {
            ...
            votecounter(votes, candidates, i);
    ...
    public static int[][] parse(String data) {
        ...
        String[][] ballot_string_arr = new String[5][250];

        String[] split_votes = data.split(";");

        for (int i = 0; i < split_votes.length; i++) {

            ballot_string_arr[0][i] = split_votes[i].substring(0, 6);
            ballot_string_arr[1][i] = split_votes[i].substring(7, 8);
            ballot_string_arr[2][i] = split_votes[i].substring(9, 10);
            ballot_string_arr[3][i] = split_votes[i].substring(11, 12);
            ballot_string_arr[4][i] = split_votes[i].substring(13);

        }

        int[][] ballot_int_arr = new int[5][250];

        for (int j = 0; j < ballot_int_arr.length; j++) {

            for (int k = 0; k < ballot_int_arr[j].length; k++) {

                ballot_int_arr[j][k] = Integer.parseInt(ballot_string_arr[j][k]);

            }

        }

        return ballot_int_arr;

    }


    public static int[][] Repeat(int[][] votes_int_arr) {

        for (int i = 0; i < votes_int_arr[0].length; i++) {

            for (int j = 0; j < votes_int_arr[0].length; j++) {

                if (votes_int_arr[0][i] == votes_int_arr[0][j] && i != j) {

                    for (int k = 1; k < votes_int_arr.length; k++) {
        ...
        return votes_int_arr;

    }

    public static void vote_num(int[][] votes, int i) {
        ...
        System.out.println("There were a total of " + Integer.toString(250 - vote_num) + " votes.\n");

    }


    public static void votecounter(int[][] votes, String[][] candidates, int a) {

        int[] vote_count = {0, 0, 0, 0, 0};

        for (int i = 0; i < vote_count.length; i++) {

            for (int j = 0; j < votes[a].length; j++) {

                if (votes[a + 1][j] == i) vote_count[i]++;
        ...
        for (int k = 1; k < candidates[a].length; k++) {
            ...
            System.out.println(candidates[a][k] + " received " + Integer.toString(vote_count[k]) + " votes.");
        ...
        for (int l = 0; l < vote_count.length; l++) {
